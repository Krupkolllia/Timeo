...
export interface Settings extends BaseRecord {
  ...
  /**
   * Раздел 6.5 ТЗ. true (по умолчанию) — «общее время» и норма периода
   * считаются по оплачиваемым часам записи (entry.hours), как и до появления
   * оплачиваемого перерыва. false — по общему времени смены (с учётом
   * перерыва). Экрана настроек (блок 7) ещё нет, поэтому переключатель временно
   * живёт на заглушке SettingsPage.
   */
  total_hours_paid_only: boolean;
}
...
export interface DayType extends BaseRecord {
  ...
  default_hours: number;
  /**
   * Раздел 5.3: времена смены по умолчанию, "ЧЧ:ММ" — тот же формат, что у
   * entry.start_time/end_time, а не «минуты от полуночи», как названо в
   * тексте раздела: lib/calc/duration.ts уже разбирает именно строки, и
   * заводить второй парсер под другое представление того же часа смысла нет
   * (см. 5.4.1). Оба поля заданы одновременно или оба пусты — раздел 6.1
   * выводит длительность только когда есть оба.
   */
  default_start: string | null;
  default_end: string | null;
  /**
   * Раздел 5.3: перерыв по умолчанию, целые минуты. Имеет смысл только вместе
   * с default_start/default_end — без времён вычитать перерыв не из чего, и
   * тип без них использует default_hours целиком (раздел 6.1).
   */
  default_break_minutes: number | null;
  /**
   * Новое понятие, которого не было в исходном ТЗ (согласовано отдельно):
   * сколько минут перерыва оплачивается. 0 — перерыв целиком неоплачиваемый
   * (поведение по умолчанию и ровно то, что было до этой работы), значение,
   * равное default_break_minutes, — перерыв оплачивается целиком, любое
   * промежуточное — оплачивается частично. Одно число, а не флаг режима: оно
   * само выражает все три состояния и не может разойтись со вторым полем,
   * которого просто нет.
   */
  default_break_paid_minutes: number | null;
...
export interface Entry extends BaseRecord {
  ...
  /**
   * Сколько минут break_minutes оплачивается — см. DayType.default_break_paid_minutes.
   * null и 0 означают одно и то же (перерыв целиком неоплачиваемый) — тот же
   * null-безопасный стиль, что у break_minutes: version(9) проставляет всем
   * существующим записям буквальный 0, а не null, потому что 0 — это то, чем
   * они и были всегда (раздел 6.1 до этой работы вычитал перерыв целиком).
   */
  paid_break_minutes: number | null;
  /**
   * Раздел 5.4: длительность вписана руками и больше не выводится из времён
   * (раздел 6.1, первая строка формулы). Экран дня показывает при этом кнопку
   * «считать по времени» — она снимает флаг и восстанавливает связь (8.2).
   *
   * У новой записи false: связь живая, и как только заданы начало и конец,
   * длительность считается по ним. У всех записей, существовавших до version(8),
   * true — ни одна из них никогда не выводилась из времён, и включать им
   * вывод задним числом значило бы переписать настоящую зарплату.
   */
