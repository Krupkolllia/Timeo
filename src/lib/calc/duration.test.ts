...
import { MINUTES_PER_DAY, deriveDurationFromTimes, parseTimeToMinutes, resolveDuration } from "@/lib/calc/duration";
...
describe("deriveDurationFromTimes — раздел 6.1", () => {
  ...
  it("инвариант 30: перерыв длиннее смены даёт ноль и предупреждение", () => {
    ...
    expect(result).toEqual({ minutes: 0, hours: 0, break_exceeds_shift: true });
  });
  ...
  it("инвариант 31: длительность — настенное время, перевод стрелок не влияет", () => {
    ...
    expect(deriveDurationFromTimes.length).toBe(3);
    expect(deriveDurationFromTimes("22:00", "06:00", null)?.minutes).toBe(480);
...
describe("resolveDuration — раздел 6.1 целиком", () => {
  ...
  const base = { start_time: null, end_time: null, break_minutes: null, duration_is_manual: false, hours: 0 };
  ...
  it("ручная длительность побеждает времена", () => {
    ...
    ).toEqual({ hours: 5, derived: false, break_exceeds_shift: false, can_derive: true });
  });
